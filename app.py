...
<div class="auth-overlay" id="auth-modal"><button class="icon auth-close" type="button" data-auth-close>×</button><section class="auth-card"><h2 id="auth-title">Entrar</h2><form id="auth-form"><input name="phone" id="auth-phone" inputmode="numeric" autocomplete="tel" placeholder="Número de telefone"><div class="auth-guide" id="auth-guide"><p id="auth-guide-copy">Para liberar sua biblioteca, informe seu ID do Kwai.</p><input name="kwai_id" id="auth-kwai" autocomplete="username" placeholder="@seu_id_no_kwai"><input name="display_name" id="auth-display-name" autocomplete="name" placeholder="Nome do criador"></div><button class="auth-submit" type="submit" id="auth-submit">Entrar</button><div class="auth-switch"><span id="auth-switch-copy">Ainda não tem conta?</span> <button type="button" data-auth-toggle id="auth-toggle">Criar conta</button></div></form></section></div>
...
function authCopy(){{const zh=lang==="zh";return {{login:zh?"登录":"Entrar",register:zh?"注册":"Criar conta",phone:zh?"请输入手机号":"Digite seu telefone",loginSubmit:zh?"登录":"Entrar",registerSubmit:zh?"完成注册":"Criar conta",missing:zh?"请输入手机号":"Digite seu telefone",notFound:zh?"没有找到账号，请先注册。":"Conta não encontrada. Crie uma conta primeiro.",registerFailed:zh?"注册失败，请重试":"Nao foi possivel criar a conta",registered:zh?"注册成功，请用手机号登录。":"Conta criada. Entre com seu telefone.",switchToRegister:zh?"如果没有账号，点击注册":"Ainda não tem conta? Clique para criar",switchToLogin:zh?"已有账号？点击登录":"Ja tem conta? Clique para entrar",profileTitle:zh?"完善 Kwai 信息":"Complete seu perfil Kwai",profileCopy:zh?"请输入你的 Kwai ID 和创作者名称，之后系统会把回传自动归到你的账号。":"Informe seu ID do Kwai e o nome do criador. Assim a Koko consegue organizar seus envios automaticamente.",kwaiMissing:zh?"请输入 Kwai ID":"Informe seu ID do Kwai",profileSubmit:zh?"开始使用":"Comecar a usar"}}}}
function accountNeedsProfile(account){{return !String(account?.kwai_id||"").trim()}}
function setAuthStep(nextStep){{authMode=nextStep==="profile"?"profile":nextStep==="register"?"register":"login";const c=authCopy();const guide=document.querySelector("#auth-guide");const switchCopy=document.querySelector("#auth-switch-copy");const toggle=document.querySelector("#auth-toggle");document.querySelector("#auth-title").textContent=authMode==="profile"?c.profileTitle:authMode==="register"?c.register:c.login;document.querySelector("#auth-submit").textContent=authMode==="profile"?c.profileSubmit:authMode==="register"?c.registerSubmit:c.loginSubmit;document.querySelector("#auth-phone").placeholder=c.phone;document.querySelector("#auth-phone").disabled=authMode==="profile";document.querySelector("#auth-guide-copy").textContent=c.profileCopy;guide?.classList.toggle("active",authMode==="profile");if(switchCopy)switchCopy.textContent=authMode==="register"?c.switchToLogin:c.switchToRegister;if(toggle){{toggle.textContent=authMode==="register"?c.login:c.register;toggle.dataset.authToggle=authMode==="register"?"login":"register";toggle.closest(".auth-switch")?.classList.toggle("hidden",authMode==="profile")}}}}
...
function uploadGuide(){{return `<div class="upload-guide">${{esc(lang==="zh"?"完成的视频需要在最下方完成上传":"O vídeo finalizado deve ser enviado no final da página.")}}</div>`}}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/auth/login":
            ...
            account = find_account(account_id)
            if not account:
                self.send_json({"error": "Conta não encontrada. Crie uma conta primeiro."}, status=404)
                return
            if not account or str(account.get("status") or "active") != "active":
