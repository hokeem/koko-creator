...
function savedList(k){{return (workspace[k]||[]).map(entry).filter(Boolean)}} function savedTabsHtml(){{return [["finished",t("statusFinished"),submissions.length],["saved",t("statusSaved"),(workspace.saved||[]).length],["schedule",lang==="zh"?"拍摄日历":"Calendario de gravacao",scheduleCount()]].map(([id,txt,count])=>`<button class="${{savedTab===id?"active":""}}" data-tab="${{id}}">${{txt}} ${{count}}</button>`).join("")}} async function renderSaved(){{document.querySelector("#schedule-mini-label").textContent=lang==="zh"?"拍摄日历":"Calendario de gravacao";document.querySelector("#saved-tabs").innerHTML=savedTabsHtml();await Promise.all([ensure(),loadSubmissions()]);const savedIds=savedTab==="schedule"?Object.values(workspace.schedule||{{}}).flat():workspace.saved||[];await ensureEntryIds(savedIds);updateProfileHeader();document.querySelector("#saved-tabs").innerHTML=savedTabsHtml();if(savedTab==="finished"){{renderSubmissionFeed();return}}if(savedTab==="schedule"){{renderScheduleFeed();return}}renderMasonry("#saved-feed",savedList("saved"))}}
