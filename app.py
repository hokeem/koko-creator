...
async function renderDashboard(){{const root=document.querySelector("#dashboard-feed");root.innerHTML=`<section class="state card"><h3>Loading...</h3></section>`;try{{await ensure(48);await loadSubmissions();const candidates=todayCandidates();if(!candidates.length){{root.innerHTML=`<section class="mission-integrated">${{preferenceStripHtml()}}<section class="state card"><h3>${{lang==="zh"?"没有匹配的脚本":"Nenhum roteiro compatível"}}</h3><p class="lead">${{lang==="zh"?"当前标签下暂时没有可推荐脚本，请重新选择标签。":"Não encontramos roteiros para suas preferências atuais. Ajuste suas preferências para receber recomendações."}}</p><button class="primary" type="button" data-reselect>${{lang==="zh"?"重新选择标签":"Mudar preferências"}}</button></section></section>`;return}}root.innerHTML=`<section class="mission-integrated">${{preferenceStripHtml()}}${{rewardBanner()}}<div id="featured-slot" class="featured-slot"></div><div id="today-plan-slot"></div></section>`;renderFeaturedAndPlan();setTimeout(openMissionGuide,260);setTimeout(scheduleCreatorOnboarding,900)}}catch(e){{root.innerHTML=`<section class="state card"><h3>Erro</h3></section>`}}}}
