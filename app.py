...
<div class="auth-overlay" id="auth-modal"><button class="icon auth-close" type="button" data-auth-close>×</button><section class="auth-card"><h2 id="auth-title">Entrar</h2><form id="auth-form"><input name="phone" id="auth-phone" inputmode="numeric" autocomplete="tel" placeholder="Número de telefone"><div class="auth-guide" id="auth-guide"><p id="auth-guide-copy">Para liberar sua biblioteca, informe seu ID do Kwai.</p><input name="kwai_id" id="auth-kwai" autocomplete="username" placeholder="@seu_id_no_kwai"><input name="display_name" id="auth-display-name" autocomplete="name" placeholder="Nome do criador"><textarea name="reason" id="auth-reason" placeholder="Por que você quer acessar a Koko?"></textarea></div><button class="auth-submit" type="submit" id="auth-submit">Entrar</button><div class="auth-switch"><span id="auth-switch-copy">Ainda não tem conta?</span> <button type="button" data-auth-toggle id="auth-toggle">Solicitar acesso</button></div></form></section></div>
