...
def public_questions_with_other() -> list[dict[str, Any]]:
    ...
    skip_options = {"flex", "unsure_scene", "hot"}
...
function detailCover(e){{return `<div class="detail-cover"><img src="${{storyboardDemoUrl}}" loading="eager" alt="Storyboard"></div>`}}
...
function shouldUsePortugueseDemo(raw){{return /[\u4e00-\u9fff]/.test(String(raw||""))}}
function extractScriptData(raw,e){{const doc=new DOMParser().parseFromString(String(raw||""),"text/html");const data={{original:e.video_url||"",main:e.summary||"",points:[],adaptable:[],segments:[],images:collectReferenceImages(doc,e)}};doc.querySelectorAll("tr").forEach(tr=>{{const cells=[...tr.children].map(td=>compactText(td.textContent));if(cells.length<2)return;const key=normalizeLabel(cells[0]);const val=cells.slice(1).join(" ");if(/video original|original/.test(key))data.original=val||data.original;if(/conteudo principal|contenido principal|内容|整体/.test(key))data.main=val||data.main;if(/pontos principais|points?|看点|爆点|重点/.test(key))data.points.push(val);if(/partes.*adapt|adaptadas|适配|替换/.test(key))data.adaptable.push(val);}});doc.querySelectorAll("table").forEach(table=>{{const rows=[...table.querySelectorAll("tr")].map(tr=>[...tr.children].map(td=>compactText(td.textContent))).filter(r=>r.length);const headIndex=rows.findIndex(r=>r.some(c=>/tempo|时间/i.test(c))&&r.some(c=>/imagem|画面|image/i.test(c)));if(headIndex<0)return;const heads=rows[headIndex].map(normalizeLabel);const idx=n=>heads.findIndex(h=>n.some(x=>h.includes(x)));const ti=idx(["tempo","时间"]), im=idx(["imagem","画面","image"]), ac=idx(["acoes","动作","action"]), di=idx(["dialogos","台词","对白","dialogue"]);rows.slice(headIndex+1).forEach(r=>{{if(ti<0||!r[ti])return;data.segments.push({{time:r[ti]||"",image:im>=0?r[im]||"":"",action:ac>=0?r[ac]||"":"",dialogue:di>=0?r[di]||"":""}})}})}});if(!data.points.length&&data.main)data.points=[data.main];if(!data.adaptable.length)data.adaptable=["O enredo pode ser adaptado mantendo a fraqueza fingida, o cuidado do marido e a virada final."];if(shouldUsePortugueseDemo(raw)){{const demo=portugueseDemoScript(e);demo.images=data.images&&data.images.length?data.images:demo.images;return demo}}return data}}
function splitBrief(list){{return list.flatMap(x=>String(x||"").split(/(?:\\n|；|;|\d+[.、])/).map(compactText).filter(Boolean)).slice(0,6)}}
...
function scriptTableRows(segs){{return segs.map((s,i)=>{{const sx=i%3,sy=Math.floor((i%9)/3);return `<tr><td class="time-cell">${{timeCellText(s.time)}}</td><td><div class="shot-cell"><div class="shot-thumb" style="--sx:${{sx}};--sy:${{sy}}"><img src="${{storyboardDemoUrl}}" alt="Storyboard frame"></div><div class="shot-text">${{esc(s.image)}}</div></div></td><td>${{esc(s.action)}}</td><td>${{esc(s.dialogue)}}</td></tr>`}}).join("")}}
function cleanScriptHtml(raw,e){{const d=extractScriptData(raw,e);const frames=(d.segments.length?d.segments:[{{time:"00:00-00:05"}},{{time:"00:05-00:10"}},{{time:"00:10-00:15"}},{{time:"00:15-00:20"}}]).slice(0,4);const brief=[{{label:"Video original",value:d.original}},{{label:"Conteúdo principal",value:d.main}},{{label:"Pontos principais",value:splitBrief(d.points).join("; ")}},{{label:"Partes que podem ser adaptadas",value:splitBrief(d.adaptable).join("; ")}}].filter(x=>x.value);const segs=d.segments.slice(0,16);return `<article class="script-html"><div class="clean-script"><section class="brief-list">${{brief.map(x=>`<div class="brief-card"><b>${{esc(x.label)}}</b><p>${{esc(x.value)}}</p></div>`).join("")}}</section>${{segs.length?`<section class="script-table-card"><div class="script-table-title">Tabela do roteiro</div><table class="script-table"><colgroup><col class="col-time"><col class="col-image"><col class="col-action"><col class="col-dialogue"></colgroup><thead><tr><th>Tempo</th><th>Imagem</th><th>Ações</th><th>Diálogos</th></tr></thead><tbody>${{scriptTableRows(segs)}}</tbody></table></section>`:""}}</div></article>`}}
