...
function coverImage(e){{return String(e.preview_image_url||e.cover_url||e.thumbnail_url||storyboardDemoUrl||"").trim()}}
...
function preferPortugueseText(s){{let text=compactText(s);if(!text)return "";if(hasChinese(text)&&/[A-Za-zÀ-ÿ]/.test(text))text=text.split(/[\u4e00-\u9fff]/)[0].trim();return text.replace(/[，。；、：！？][^A-Za-zÀ-ÿ]*$/,"").trim()}}
...
function extractScriptData(raw,e){{const doc=new DOMParser().parseFromString(String(raw||""),"text/html");const data={{original:e.video_url||"",main:preferPortugueseText(e.summary)||"",points:[],adaptable:[],segments:[],images:collectReferenceImages(doc,e)}};doc.querySelectorAll("tr").forEach(tr=>{{const cells=[...tr.children].map(td=>preferPortugueseText(td.textContent));if(cells.length<2)return;const key=normalizeLabel(cells[0]);const val=cells.slice(1).join(" ").trim();if(/video original|original/.test(key))data.original=val||data.original;if(/conteudo principal|contenido principal|内容|整体/.test(key))data.main=val||data.main;if(/pontos principais|points?|看点|爆点|重点/.test(key))data.points.push(val);if(/partes.*adapt|adaptadas|适配|替换/.test(key))data.adaptable.push(val);}});doc.querySelectorAll("table").forEach(table=>{{const rows=[...table.querySelectorAll("tr")].map(tr=>[...tr.children].map(td=>preferPortugueseText(td.textContent))).filter(r=>r.length);const headIndex=rows.findIndex(r=>r.some(c=>/tempo|时间/i.test(c))&&(r.some(c=>/imagem|画面|image/i.test(c))||r.length>=4));if(headIndex<0)return;const heads=rows[headIndex].map(normalizeLabel);const idx=n=>heads.findIndex(h=>n.some(x=>h.includes(x)));let ti=idx(["tempo","时间"]), im=idx(["imagem","画面","image"]), ac=idx(["acoes","动作","action"]), di=idx(["dialogos","台词","对白","dialogue"]);if(ti<0&&heads.length>=4){{ti=0;im=1;ac=2;di=3}}rows.slice(headIndex+1).forEach(r=>{{if(ti<0||!r[ti])return;data.segments.push({{time:r[ti]||"",image:im>=0?r[im]||"":"",action:ac>=0?r[ac]||"":"",dialogue:di>=0?r[di]||"":""}})}})}});data.points=splitBrief(data.points).filter(x=>x&&x!==data.main);data.adaptable=splitBrief(data.adaptable);return data}}
...
function scriptTableRows(segs,cover){{return segs.map((s,i)=>{{const sx=i%3,sy=Math.floor((i%9)/3);return `<tr><td class="time-cell">${{timeCellText(s.time)}}</td><td><div class="shot-cell"><div class="shot-thumb" style="--sx:${{sx}};--sy:${{sy}}"><img src="${{esc(cover)}}" alt="Storyboard frame"></div><div class="shot-text">${{esc(s.image)}}</div></div></td><td>${{esc(s.action)}}</td><td>${{esc(s.dialogue)}}</td></tr>`}}).join("")}}
function cleanScriptHtml(raw,e){{const d=extractScriptData(raw,e);const brief=[{{label:"Video original",value:d.original}},{{label:"Conteúdo principal",value:d.main}},{{label:"Pontos principais",value:d.points.join("; ")}},{{label:"Partes que podem ser adaptadas",value:d.adaptable.join("; ")}}].filter(x=>x.value);const segs=d.segments.slice(0,16);const cover=coverImage(e);return `<article class="script-html"><div class="clean-script"><section class="brief-list">${{brief.map(x=>`<div class="brief-card"><b>${{esc(x.label)}}</b><p>${{esc(x.value)}}</p></div>`).join("")}}</section>${{segs.length?`<section class="script-table-card"><div class="script-table-title">Tabela do roteiro</div><table class="script-table"><colgroup><col class="col-time"><col class="col-image"><col class="col-action"><col class="col-dialogue"></colgroup><thead><tr><th>Tempo</th><th>Imagem</th><th>Ações</th><th>Diálogos</th></tr></thead><tbody>${{scriptTableRows(segs,cover)}}</tbody></table></section>`:""}}</div></article>`}}
