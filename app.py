...
*{{box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}}body{{margin:0;background:#fff4ea;color:#1f1f1f}}button,a{{font:inherit}}.phone{{width:min(100%,480px);min-height:100vh;margin:0 auto;padding-bottom:96px;overflow-x:hidden;background:linear-gradient(180deg,#fffaf5,#fff0df 42%,#fff8f2)}}.top{{position:sticky;top:0;z-index:10;display:flex;align-items:center;justify-content:space-between;padding:18px 22px 12px;background:rgba(255,252,248,.9);backdrop-filter:blur(16px)}}.brand{{font-size:34px;font-weight:900}}.brand span{{color:#ff5f00;font-size:17px;margin-left:8px}}.lang{{position:fixed;right:max(14px,calc((100vw - 480px)/2 + 14px));bottom:92px;z-index:20;display:flex;gap:4px;padding:5px;border-radius:999px;background:white;box-shadow:0 12px 28px #ff820022}}.lang button{{border:0;border-radius:999px;padding:7px 10px;background:transparent;font-size:12px;font-weight:850;color:#777}}.lang .active{{background:#ff5f00;color:white}}.view{{display:none;padding:22px}}.view.active{{display:block}}.pill,.chip,.tag{{display:inline-flex;align-items:center;border:1px solid #ff5f0070;border-radius:999px;padding:8px 12px;color:#ff5f00;background:#ffffff90;font-size:12px;font-weight:850}}h1{{margin:18px 0 12px;font-size:clamp(38px,10vw,56px);line-height:1.08;font-weight:900}}.lead{{margin:0;color:#69707a;font-size:16px;line-height:1.55}}.primary,.open{{border:0;border-radius:999px;min-height:48px;padding:0 16px;display:inline-flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(90deg,#ff6a00,#ff5200);color:white;text-decoration:none;font-weight:900;box-shadow:0 14px 30px #ff5f0040}}.secondary{{border:0;border-radius:999px;min-height:44px;padding:0 16px;background:white;color:#1f1f1f;font-weight:850;box-shadow:0 10px 24px #00000010}}.cta{{display:grid;gap:12px;margin:18px 0}}.card{{border-radius:22px;background:#ffffffdd;border:1px solid #ff82001a;box-shadow:0 16px 38px #552d0a14}}.hero{{min-height:150px;margin:20px -22px 0;position:relative;overflow:hidden}}.mascot{{position:absolute;right:26px;bottom:8px;width:116px;height:116px;border-radius:52% 48% 44% 56%;background:radial-gradient(circle at 35% 22%,#ffbe55,#ff8e24 64%,#f97808)}}.quick{{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}}.quick button{{min-height:78px;border:0;border-radius:18px;background:white;font-weight:850}}.quick b{{display:block;color:#ff5f00;font-size:22px}}.stepper{{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:18px 0}}.step{{min-height:58px;border-radius:18px;background:white;display:grid;place-items:center;color:#777;font-weight:900}}.step.active{{background:#ff5f00;color:white}}.options,.feed{{display:grid;gap:14px;margin-top:16px}}.option{{min-height:72px;border:1px solid #ff820026;border-radius:18px;background:white;text-align:left;padding:14px;font-weight:850}}.option.selected{{border-color:#ff5f00;color:#ff5f00}}.script{{display:grid;grid-template-columns:116px 1fr;gap:13px;padding:14px;min-height:168px}}.thumb{{position:relative;overflow:hidden;border-radius:16px;min-height:142px;background:#2a1d16;color:white;padding:10px;font-size:12px;font-weight:900}}.thumb img{{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}}.thumb:after{{content:"";position:absolute;inset:0;background:linear-gradient(180deg,#00000010,#000000aa)}}.thumb span{{position:relative;z-index:1;background:#9e490ce0;border-radius:9px;padding:6px 8px}}.body{{min-width:0;display:flex;flex-direction:column;gap:8px}}.body h3{{margin:0;font-size:18px;line-height:1.22;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}}.body p{{margin:0;color:#69707a;font-size:13px;line-height:1.42;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}}.tags{{display:flex;gap:6px;flex-wrap:wrap}}.tag{{padding:5px 8px;background:#fff0e8;font-size:11px}}.actions{{display:grid;grid-template-columns:1fr 38px 38px;gap:8px;margin-top:auto}}.icon{{border:0;width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:#fff0e8;color:#ff5f00;font-weight:900}}.tabs{{display:flex;gap:8px;overflow:auto;padding:4px 0 12px}}.tabs button{{border:1px solid #ff5f0038;border-radius:999px;padding:9px 13px;background:white;color:#777;font-size:12px;font-weight:850}}.tabs .active{{background:#ff5f00;color:white}}.bottom{{position:fixed;left:50%;bottom:0;transform:translateX(-50%);z-index:18;width:min(100%,480px);display:grid;grid-template-columns:repeat(4,1fr);gap:2px;padding:10px 14px;background:#fffffff0;border-radius:24px 24px 0 0;box-shadow:0 -14px 34px #00000014}}.bottom button{{border:0;background:transparent;min-height:54px;color:#777;font-size:12px;font-weight:750}}.bottom .active{{color:#ff5f00}}.modal{{position:fixed;inset:0;z-index:50;display:none;align-items:flex-end;background:#1f1f1f55;padding:18px 18px 0}}.modal.active{{display:flex}}.sheet{{width:min(100%,480px);max-height:88vh;overflow:auto;margin:0 auto;border-radius:28px 28px 0 0;background:#fffaf5;padding:18px}}.sheet-img{{height:220px;border-radius:20px;overflow:hidden;background:#2a1d16}}.sheet-img img{{width:100%;height:100%;object-fit:cover}}.submit{{display:grid;gap:10px;margin:14px 0;padding:14px;border-radius:18px;background:#fff0e8}}.submit input{{min-height:46px;border:1px solid #ff5f0038;border-radius:14px;padding:0 12px}}.state{{padding:18px}}@media(max-width:380px){{.view{{padding:18px}}h1{{font-size:36px}}.script{{grid-template-columns:104px 1fr}}}}
</style></head><body><main class="phone"><header class="top"><div class="brand">kwai <span>Koko</span></div><button class="icon" type="button">☰</button></header><div class="lang"><button data-lang="pt">PT</button><button data-lang="zh">中文</button></div>
<section class="view" data-view="home"><div class="pill" data-t="homePill">Biblioteca de roteiros</div><h1 data-html="homeTitle">Encontre roteiros que você consegue gravar</h1><p class="lead" data-t="homeLead">Responda 3 perguntas e veja roteiros para o seu estilo.</p><div class="hero"><div class="mascot"></div></div><div class="cta"><button class="primary" data-go="choose" data-t="start">Começar agora</button><button class="secondary" data-go="library" data-t="seePopular">Ver populares</button></div></section>
<section class="view" data-view="dashboard"><div class="pill" data-t="todayPill">Seu painel Koko</div><h1 data-t="todayTitle">Roteiros novos para o seu estilo</h1><p class="lead" data-t="todayLead">Abra, salve e marque o que você vai gravar hoje.</p><div class="quick"><button data-go="library"><b id="count-new">0</b><span data-t="quickNew">novos</span></button><button data-go="saved"><b id="count-saved">0</b><span data-t="quickSaved">salvos</span></button><button data-go="saved" data-saved-tab="planned"><b id="count-planned">0</b><span data-t="quickPlan">para gravar</span></button></div><div class="feed" id="dashboard-feed"></div></section>
<section class="view" data-view="choose"><div class="pill" id="step-label">Etapa 1 de 3</div><div class="stepper" id="stepper"></div><div id="question"></div><button class="primary" id="next-step"><span data-t="next">Próxima etapa</span> →</button></section>
<section class="view" data-view="library"><div class="pill" data-t="libraryPill">Biblioteca</div><h1 data-t="libraryTitle">Sua biblioteca recomendada</h1><div class="tags" id="filters"></div><div class="feed" id="library-feed"></div></section>
<section class="view" data-view="saved"><div class="pill" data-t="savedPill">Meus roteiros</div><h1 data-t="savedTitle">Sua lista de gravação</h1><div class="tabs" id="saved-tabs"></div><div class="feed" id="saved-feed"></div></section></main>
<nav class="bottom"><button data-go="dashboard">⌂<br><span data-t="navHome">Início</span></button><button data-go="library">▥<br><span data-t="navLibrary">Biblioteca</span></button><button data-go="saved">♡<br><span data-t="navSaved">Salvos</span></button><button data-go="choose">⚙<br><span data-t="navPrefs">Perfil</span></button></nav>
<div class="modal" id="modal"><section class="sheet"><button class="icon" data-close>×</button><div id="detail"></div></section></div>
<script>
...
const I={{pt:{{homePill:"Biblioteca de roteiros",homeTitle:"Encontre roteiros que você consegue gravar",homeLead:"Responda 3 perguntas e veja roteiros para o seu estilo.",start:"Começar agora",seePopular:"Ver populares",todayPill:"Seu painel Koko",todayTitle:"Roteiros novos para o seu estilo",todayLead:"Abra, salve e marque o que você vai gravar hoje.",quickNew:"novos",quickSaved:"salvos",quickPlan:"para gravar",next:"Próxima etapa",finish:"Entrar na biblioteca",libraryPill:"Biblioteca",libraryTitle:"Sua biblioteca recomendada",savedPill:"Meus roteiros",savedTitle:"Sua lista de gravação",navHome:"Início",navLibrary:"Biblioteca",navSaved:"Salvos",navPrefs:"Perfil",open:"Abrir",save:"Salvar",plan:"Vou gravar",done:"Gravado",reject:"Não serve",original:"Vídeo",details:"Detalhes",submitTitle:"Enviar vídeo gravado",submitHint:"Envie o link do vídeo gravado seguindo este roteiro. Vamos revisar e, se aprovado, ajudar com impulsionamento.",submitPlaceholder:"Cole aqui o link do seu vídeo",submitButton:"Enviar para revisão",submitOk:"Recebido. Vamos revisar seu vídeo.",submitError:"Não foi possível enviar. Confira o link.",empty:"Nada aqui ainda",emptyText:"Salve um roteiro da biblioteca para montar sua lista.",statusSaved:"Salvos",statusPlanned:"Vou gravar",statusFinished:"Gravados",statusRejected:"Não servem",step:"Etapa"}},zh:{{homePill:"脚本库",homeTitle:"找到你真的能拍的脚本",homeLead:"回答 3 个问题，进入你的推荐脚本库。",start:"开始选择",seePopular:"先看热门",todayPill:"Koko 工作台",todayTitle:"适合你风格的新脚本",todayLead:"打开、收藏，并标记今天准备拍的脚本。",quickNew:"新脚本",quickSaved:"已收藏",quickPlan:"准备拍",next:"下一步",finish:"进入脚本库",libraryPill:"脚本库",libraryTitle:"你的推荐脚本库",savedPill:"我的脚本",savedTitle:"你的拍摄清单",navHome:"首页",navLibrary:"脚本库",navSaved:"收藏",navPrefs:"偏好",open:"打开",save:"收藏",plan:"准备拍",done:"已拍",reject:"不适合",original:"原视频",details:"完整脚本",submitTitle:"回传拍摄视频",submitHint:"上传按照脚本拍摄的视频，我们会审核后给您投流。",submitPlaceholder:"把你发布后的视频链接粘贴在这里",submitButton:"提交审核",submitOk:"已收到，我们会审核这个视频。",submitError:"提交失败，请检查链接。",empty:"这里还没有脚本",emptyText:"先从脚本库收藏一个脚本。",statusSaved:"已收藏",statusPlanned:"准备拍",statusFinished:"已拍",statusRejected:"不适合",step:"第"}}}};
...
function counts(){{document.querySelector("#count-new").textContent=String(entries.length);document.querySelector("#count-saved").textContent=String((workspace.saved||[]).length);document.querySelector("#count-planned").textContent=String((workspace.planned||[]).length)}}
...
function show(v){{if(["dashboard","library","saved"].includes(v)&&!hasProfile())v="home";document.querySelectorAll("[data-view]").forEach(x=>x.classList.toggle("active",x.dataset.view===v));if(v==="dashboard")renderDashboard();if(v==="library")renderLibrary();if(v==="saved")renderSaved();document.querySelectorAll(".bottom button").forEach(b=>b.classList.toggle("active",b.dataset.go===v));scrollTo({{top:0,behavior:"smooth"}})}}
function renderQuestion(){{const q=questions[step];document.querySelector("#step-label").textContent=lang==="zh"?`${{t("step")}} ${{step+1}} / 3`:`${{t("step")}} ${{step+1}} de 3`;document.querySelector("#stepper").innerHTML=questions.map((_,i)=>`<div class="step ${{i===step?"active":""}}">${{i+1}}</div>`).join("");document.querySelector("#question").innerHTML=`<h1>${{esc(label(q))}}</h1><div class="options">${{q.options.map(o=>`<button class="option ${{answers[q.id]===o.id?"selected":""}}" data-answer="${{q.id}}" data-value="${{o.id}}">${{esc(label(o))}}</button>`).join("")}}</div>`;document.querySelector("#next-step span").textContent=step===questions.length-1?t("finish"):t("next")}}
...
function renderList(sel,list){{document.querySelector(sel).innerHTML=list.length?list.map(card).join(""):`<section class="state card"><h3>${{t("empty")}}</h3><p class="lead">${{t("emptyText")}}</p><button class="primary" data-go="library">${{t("navLibrary")}}</button></section>`}}
async function ensure(){{if(!entries.length)await loadEntries()}} async function renderDashboard(){{document.querySelector("#dashboard-feed").innerHTML=`<section class="state card"><h3>Loading...</h3></section>`;try{{await ensure();renderList("#dashboard-feed",entries.slice(0,5))}}catch(e){{document.querySelector("#dashboard-feed").innerHTML=`<section class="state card"><h3>Erro</h3></section>`}}}}
async function renderLibrary(){{document.querySelector("#filters").innerHTML=chips()+`<button class="chip" data-go="choose">${{lang==="zh"?"重新筛选":"Filtrar novamente"}}</button>`;document.querySelector("#library-feed").innerHTML=`<section class="state card"><h3>Loading...</h3></section>`;try{{await loadEntries();renderList("#library-feed",entries)}}catch(e){{document.querySelector("#library-feed").innerHTML=`<section class="state card"><h3>Erro</h3></section>`}}}}
...
function renderCurrent(){{const v=document.querySelector(".view.active")?.dataset.view;if(v==="dashboard")renderDashboard();if(v==="library")renderLibrary();if(v==="saved")renderSaved()}}
function openDetail(id){{const e=entry(id);if(!e)return;document.querySelector("#detail").innerHTML=`<div class="sheet-img"><img src="${{esc(e.thumbnail_url)}}" alt=""></div><h2>${{esc(e.title)}}</h2><div class="tags"><span class="tag">${{esc(e.content_type)}}</span><span class="tag">1-3 min</span></div><p class="lead">${{esc(e.summary)}}</p><section class="submit"><b>${{t("submitTitle")}}</b><p class="lead">${{t("submitHint")}}</p><input type="url" data-submit-url="${{esc(e.entry_id)}}" placeholder="${{t("submitPlaceholder")}}"><button class="primary" data-submit="${{esc(e.entry_id)}}">${{t("submitButton")}}</button><div id="submit-status-${{esc(e.entry_id)}}"></div></section><div class="cta"><button class="primary" data-status="planned" data-entry="${{esc(e.entry_id)}}">${{t("plan")}}</button><button class="secondary" data-status="saved" data-entry="${{esc(e.entry_id)}}">${{t("save")}}</button><button class="secondary" data-status="finished" data-entry="${{esc(e.entry_id)}}">${{t("done")}}</button><button class="secondary" data-status="rejected" data-entry="${{esc(e.entry_id)}}">${{t("reject")}}</button>${{e.html_url?`<a class="primary" target="_blank" href="${{esc(e.html_url)}}">▷ ${{t("details")}}</a>`:""}}${{e.video_url?`<a class="secondary" target="_blank" href="${{esc(e.video_url)}}">${{t("original")}}</a>`:""}}</div>`;document.querySelector("#modal").classList.add("active")}}
...
document.addEventListener("click",e=>{{const l=e.target.closest("[data-lang]");if(l){{lang=l.dataset.lang;localStorage.setItem(langKey,lang);applyLang();return}}const tab=e.target.closest("[data-tab]");if(tab){{savedTab=tab.dataset.tab;renderSaved();return}}const d=e.target.closest("[data-detail]");if(d){{openDetail(d.dataset.detail);return}}if(e.target.closest("[data-close]")||e.target.id==="modal"){{document.querySelector("#modal").classList.remove("active");return}}const sub=e.target.closest("[data-submit]");if(sub){{submitVideo(sub.dataset.submit);return}}const st=e.target.closest("[data-status]");if(st){{setStatus(st.dataset.entry,st.dataset.status);return}}const go=e.target.closest("[data-go]");if(go){{if(go.dataset.savedTab)savedTab=go.dataset.savedTab;show(go.dataset.go);return}}const ans=e.target.closest("[data-answer]");if(ans){{answers[ans.dataset.answer]=ans.dataset.value;saveProfile();renderQuestion();return}}if(e.target.closest("#next-step")){{if(step<questions.length-1){{step++;renderQuestion()}}else{{saveProfile();show("dashboard")}}}}}});
applyLang();show(hasProfile()?"dashboard":"home");
</script></body></html>"""
