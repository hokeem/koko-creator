...
<div class="auth-overlay" id="auth-modal"><button class="icon auth-close" type="button" data-auth-close>×</button><section class="auth-card"><h2 id="auth-title">Registrar</h2><form id="auth-form"><input name="phone" id="auth-phone" inputmode="numeric" autocomplete="tel" placeholder="Número de telefone"><div class="auth-guide" id="auth-guide"><p id="auth-guide-copy">Para liberar sua biblioteca, informe seu ID do Kwai.</p><input name="kwai_id" id="auth-kwai" autocomplete="username" placeholder="@seu_id_no_kwai"><input name="display_name" id="auth-display-name" autocomplete="name" placeholder="Nome do criador"></div><button class="auth-submit" type="submit" id="auth-submit">Registrar</button></form></section></div>
...
function authCopy(){{const zh=lang==="zh";return {{login:zh?"注册":"Registrar",phone:zh?"请输入手机号":"Digite seu telefone",submit:zh?"注册":"Registrar",missing:zh?"请输入手机号":"Digite seu telefone",notFound:zh?"注册失败，请重试":"Nao foi possivel registrar",profileTitle:zh?"完善 Kwai 信息":"Complete seu perfil Kwai",profileCopy:zh?"请输入你的 Kwai ID 和创作者名称，之后系统会把回传自动归到你的账号。":"Informe seu ID do Kwai e o nome do criador. Assim a Koko consegue organizar seus envios automaticamente.",kwaiMissing:zh?"请输入 Kwai ID":"Informe seu ID do Kwai",profileSubmit:zh?"开始使用":"Comecar a usar"}}}}
function accountNeedsProfile(account){{return !String(account?.kwai_id||"").trim()}}
function setAuthStep(nextStep){{authMode=nextStep==="profile"?"profile":"login";const c=authCopy();const guide=document.querySelector("#auth-guide");document.querySelector("#auth-title").textContent=authMode==="profile"?c.profileTitle:c.login;document.querySelector("#auth-submit").textContent=authMode==="profile"?c.profileSubmit:c.submit;document.querySelector("#auth-phone").placeholder=c.phone;document.querySelector("#auth-phone").disabled=authMode==="profile";document.querySelector("#auth-guide-copy").textContent=c.profileCopy;guide?.classList.toggle("active",authMode==="profile")}}
function setAuthMode(mode){{setAuthStep("login")}}
function openAuth(mode="login"){{setAuthStep("login");document.querySelector("#auth-modal").classList.add("active")}}
...
async function handleAuthSubmit(e){{e.preventDefault();const form=new FormData(e.currentTarget);const phone=String(form.get("phone")||"").replace(/\\s+/g,"").trim();const c=authCopy();if(!phone){{alert(c.missing);return}}try{{if(authMode==="profile"){{const kwai_id=String(form.get("kwai_id")||"").trim();const display_name=String(form.get("display_name")||"").trim();if(!kwai_id){{alert(c.kwaiMissing);return}}const r=await fetch("/api/me/profile",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{kwai_id,display_name}})}});const d=await r.json();if(!r.ok)throw new Error(d.error||c.notFound);creatorUser=d.account;localStorage.setItem(authKey,JSON.stringify(creatorUser));await loadAccountState();updateCreatorName();closeAuth();if(!hasProfile())show("choose");else show("dashboard");return}}const r=await fetch("/api/auth/login",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{phone}})}});const d=await r.json();if(!r.ok)throw new Error(d.error||c.notFound);creatorUser=d.account;localStorage.setItem(authKey,JSON.stringify(creatorUser));await loadAccountState();updateCreatorName();document.querySelector("#auth-kwai").value=creatorUser?.kwai_id||"";document.querySelector("#auth-display-name").value=creatorUser?.display_name&&creatorUser.display_name!==phone?creatorUser.display_name:"";if(accountNeedsProfile(creatorUser)){{setAuthStep("profile");return}}closeAuth();if(!hasProfile())show("choose");else show("dashboard")}}catch(err){{alert(err.message||c.notFound)}}}}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/api/auth/login":
            ...
            if not account_id:
                self.send_json({"error": "Digite o numero de telefone"}, status=400)
                return
            account = find_account(account_id)
            if not account:
                public_new = upsert_account(
                    account_id,
                    source="self_signup",
                    display_name=account_id,
                    phone=raw_phone,
                )
                account = find_account(str(public_new.get("account_id") or account_id))
            if not account or str(account.get("status") or "active") != "active":
