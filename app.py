...
def public_script_detail(entry: dict[str, Any]) -> dict[str, Any]:
    item = public_entry(entry, 100)
    item["script_html"] = script_html_for_entry(entry)
    return item
...
<section class="view" data-view="dashboard"><h1 data-t="todayTitle">脚本推荐</h1><p class="lead" data-t="todayLead">Abra e escolha um roteiro para ver os detalhes.</p><div class="tags" id="filters"></div><div class="masonry" id="dashboard-feed"></div></section>
<section class="view" data-view="choose"><span class="step-label" id="step-label">Etapa 1 de 3</span><div class="stepper" id="stepper"></div><div id="question"></div><button class="primary" id="next-step"><span data-t="next">Próxima etapa</span> →</button></section>
...
function renderMasonry(sel,list){{document.querySelector(sel).innerHTML=list.length?list.map(masonryCard).join(""):`<section class="state card"><h3>${{t("empty")}}</h3><p class="lead">${{t("emptyText")}}</p><button class="primary" data-go="dashboard">${{t("navHome")}}</button></section>`}}
async function ensure(){{if(!entries.length)await loadEntries()}} async function renderDashboard(){{document.querySelector("#filters").innerHTML=chips()+`<button class="chip reselect" type="button" data-reselect="true">${{t("changePrefs")}}</button>`;document.querySelector("#dashboard-feed").innerHTML=`<section class="state card"><h3>Loading...</h3></section>`;try{{await loadEntries();renderMasonry("#dashboard-feed",entries)}}catch(e){{document.querySelector("#dashboard-feed").innerHTML=`<section class="state card"><h3>Erro</h3></section>`}}}}
...
async function fetchScript(id){{let e=entry(id);if(e&&e.script_html!==undefined)return e;const r=await fetch(`/api/creator/scripts/${{encodeURIComponent(id)}}?_=${{Date.now()}}`);const d=await r.json();if(!r.ok)throw new Error(d.error||"load failed");e=d.entry;if(!entries.some(x=>x.entry_id===e.entry_id))entries.unshift(e);else entries=entries.map(x=>x.entry_id===e.entry_id?{{...x,...e}}:x);return e}}
function shareUrl(id){{return `${{location.origin}}/script/${{id}}`}}
function videoPreview(e){{const url=esc(e.video_url);const img=esc(e.thumbnail_url);if(!url)return `<div class="video-box"><img src="${{img}}" alt=""></div>`;return `<div class="video-box"><iframe src="${{url}}" title="video preview" loading="lazy" allow="autoplay; encrypted-media; fullscreen; picture-in-picture" sandbox="allow-scripts allow-same-origin allow-popups allow-presentation"></iframe><div class="video-fallback">${{lang==="zh"?"如果平台禁止内嵌播放，这里可能只显示空白。":"Se a plataforma bloquear embed, o preview pode aparecer em branco."}}</div></div>`}}
async function openDetail(id){{const modal=document.querySelector("#modal");document.querySelector("#detail").innerHTML=`<div class="detail-top"><button class="icon" data-close>×</button></div><section class="state card"><h3>${{lang==="zh"?"正在加载脚本..." :"Carregando roteiro..."}}</h3></section>`;modal.classList.add("active");try{{const e=await fetchScript(id);const s=statusOf(e.entry_id);document.querySelector("#detail").innerHTML=`<div class="detail-top"><button class="icon" data-close>×</button></div>${{videoPreview(e)}}<h2 class="detail-title">${{esc(e.title)}}</h2><div class="tags"><span class="tag">${{esc(e.content_type)}}</span><span class="tag">1-3 min</span>${{s?`<span class="tag">${{s}}</span>`:""}}</div>${{e.script_html?`<article class="script-html">${{e.script_html}}</article>`:`<article class="script-html"><p>${{esc(e.summary)}}</p></article>`}}<section class="submit"><b>${{t("submitTitle")}}</b><p class="lead">${{t("submitHint")}}</p><input type="url" data-submit-url="${{esc(e.entry_id)}}" placeholder="${{t("submitPlaceholder")}}"><button class="primary" data-submit="${{esc(e.entry_id)}}">${{t("submitButton")}}</button><div id="submit-status-${{esc(e.entry_id)}}"></div></section><div class="detail-actions"><button class="secondary" data-status="${{s==="saved"?"":"saved"}}" data-entry="${{esc(e.entry_id)}}">${{s==="saved"?t("saved"):t("save")}}</button><button class="secondary" data-status="planned" data-entry="${{esc(e.entry_id)}}">${{t("plan")}}</button><button class="secondary" data-copy-share="${{esc(e.entry_id)}}">${{lang==="zh"?"复制分享链接":"Copiar link"}}</button><button class="primary" data-submit-scroll="${{esc(e.entry_id)}}">${{lang==="zh"?"回传视频":"Enviar vídeo"}}</button></div>`}}catch(err){{document.querySelector("#detail").innerHTML=`<div class="detail-top"><button class="icon" data-close>×</button></div><section class="state card"><h3>${{lang==="zh"?"脚本加载失败":"Falha ao carregar"}}</h3><p>${{esc(err.message)}}</p></section>`}}}}
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path.startswith("/api/creator/scripts/"):
            ...
            if not entry:
                ...
                return
            try:
                self.send_json({"entry": public_script_detail(entry)})
            except Exception as exc:
