...
"""
    ...
    profile_override_css += """#mission-guide.mission-popup{padding:0!important;overflow:hidden!important}#mission-guide .mission-guide-card{width:min(94vw,392px)!important;max-height:min(76vh,560px)!important;transform:scale(.7)!important;transform-origin:center center!important;border-radius:26px!important}.mission-guide-scroll-cue{position:sticky;bottom:8px;z-index:5;width:max-content;margin:-4px auto 8px;padding:6px 10px;border-radius:999px;background:#fff7f0e8;border:1px solid #ff5f0028;color:#ff5f00;font-size:10px;font-weight:950;box-shadow:0 8px 18px #552d0a14;animation:guideCue 1.25s ease-in-out infinite;backdrop-filter:blur(8px)}@keyframes guideCue{0%,100%{transform:translateY(0);opacity:.62}50%{transform:translateY(5px);opacity:1}}@media(max-width:380px){#mission-guide .mission-guide-card{width:96vw!important;max-height:min(78vh,570px)!important;transform:scale(.7)!important}.mission-guide-scroll-cue{font-size:9px!important;padding:5px 9px!important}}"""
...
function preferenceFirstCandidates(limit=10){{const picked=[];const seen=new Set();const add=list=>recommendationDateSorted(list).forEach(e=>{{if(picked.length<limit&&!seen.has(e.entry_id)){{picked.push(e);seen.add(e.entry_id)}}}});add(entries.filter(hardMatchesCurrentPreference));add(entries.filter(topicMatchesCurrentPreference));add(entries);return picked}}
function missionCandidates(){{return preferenceFirstCandidates(10)}}
...
let onboardingIndex=0;let onboardingTimer=0;let onboardingActive=false;
...
function shouldShowCreatorOnboarding(){{return !!creatorUser&&!initialScriptId&&hasProfile()&&!workspace.creatorOnboardingDone}}
...
function finishCreatorOnboarding(){{onboardingActive=false;document.querySelector("#creator-onboarding")?.classList.remove("active");workspace.creatorOnboardingDone=true;saveWorkspace()}}
function positionOnboarding(target){{const overlay=document.querySelector("#creator-onboarding");const spot=document.querySelector("#onboarding-spot");const card=document.querySelector("#onboarding-card");if(!overlay||!spot||!card||!target)return;const r=target.getBoundingClientRect();const pad=8;const left=Math.max(10,r.left-pad);const top=Math.max(10,r.top-pad);const width=Math.min(innerWidth-20,r.width+pad*2);const height=Math.min(innerHeight-20,r.height+pad*2);spot.style.left=`${{left}}px`;spot.style.top=`${{top}}px`;spot.style.width=`${{width}}px`;spot.style.height=`${{height}}px`;spot.style.borderRadius=getComputedStyle(target).borderRadius||"24px";const below=r.bottom+16;const cardHeight=Math.min(card.offsetHeight||180,230);let cardTop=below;if(cardTop+cardHeight>innerHeight-12)cardTop=Math.max(12,r.top-cardHeight-16);card.style.top=`${{cardTop}}px`}}
function renderOnboardingStep(){{const steps=onboardingCopySet();if(onboardingIndex>=steps.length){{finishCreatorOnboarding();return}}const stepData=steps[onboardingIndex];if(["plan","planDetail","upload"].includes(stepData.id))ensureOnboardingPlan();const overlay=document.querySelector("#creator-onboarding");if(!overlay)return;overlay.classList.add("active");overlay.setAttribute("aria-hidden","false");document.querySelector("#onboarding-step").textContent=`${{onboardingIndex+1}}/${{steps.length}}`;document.querySelector("#onboarding-title").textContent=stepData.title;document.querySelector("#onboarding-copy").textContent=stepData.copy;document.querySelector("[data-onboarding-back]").disabled=onboardingIndex===0;document.querySelector("[data-onboarding-next]").textContent=onboardingIndex===steps.length-1?(lang==="zh"?"完成":"Concluir"):(lang==="zh"?"下一步":"Próximo");document.querySelector("[data-onboarding-skip]").textContent=lang==="zh"?"跳过":"Pular";document.querySelector("[data-onboarding-back]").textContent=lang==="zh"?"上一步":"Voltar";setTimeout(()=>{{let target=document.querySelector(stepData.selector)||document.querySelector("#dashboard-feed")||document.body;target.scrollIntoView({{behavior:"smooth",block:"center"}});document.querySelectorAll(".onboarding-nudge").forEach(x=>x.classList.remove("onboarding-nudge"));target.classList.add("onboarding-nudge");setTimeout(()=>positionOnboarding(target),360)}},40)}}
