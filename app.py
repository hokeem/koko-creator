...
function setAuthStep(nextStep){{authMode=nextStep==="profile"?"profile":nextStep==="register"?"register":"login";const c=authCopy();const guide=document.querySelector("#auth-guide");const switchCopy=document.querySelector("#auth-switch-copy");const toggle=document.querySelector("#auth-toggle");const reason=document.querySelector("#auth-reason");const display=document.querySelector("#auth-display-name");const loginHint=document.querySelector("#auth-login-hint");document.querySelector("#auth-title").textContent=authMode==="profile"?c.profileTitle:authMode==="register"?c.register:c.login;document.querySelector("#auth-submit").textContent=authMode==="profile"?c.profileSubmit:authMode==="register"?c.registerSubmit:c.loginSubmit;document.querySelector("#auth-phone").placeholder=c.phone;document.querySelector("#auth-phone").disabled=authMode==="profile";if(loginHint){{loginHint.textContent=c.loginHint;loginHint.hidden=authMode!=="login"}}document.querySelector("#auth-guide-copy").textContent=authMode==="register"?c.applicationCopy:c.profileCopy;guide?.classList.toggle("active",authMode==="profile"||authMode==="register");if(reason)reason.hidden=authMode!=="register";if(display)display.hidden=authMode==="register";if(switchCopy)switchCopy.textContent=authMode==="register"?c.switchToLogin:c.switchToRegister;if(toggle){{toggle.textContent=authMode==="register"?c.login:c.register;toggle.dataset.authToggle=authMode==="register"?"login":"register";toggle.closest(".auth-switch")?.classList.toggle("hidden",authMode==="profile")}}}}
function setAuthMode(mode){{setAuthStep(mode==="register"?"register":"login")}}
function openAuth(mode="login"){{setAuthStep(mode==="register"?"register":"login");document.querySelector("#auth-modal").classList.add("active")}}
...
function featuredCard(e,i){{const s=statusOf(e.entry_id);const liked=ids("saved").has(e.entry_id);const tags=[ptTag(e.content_type),durationLabel(e),...(s?[ptTag(s)]:[])].filter(Boolean);return `<section class="featured-shell"><article class="featured-card"><div class="featured-media"><img src="${{esc(scriptImage(e))}}" loading="eager" alt=""><span class="featured-badge">Recomendado agora</span><span class="featured-score">${{i+1}}/${{entries.length}}</span></div><div class="featured-body"><h2 class="featured-title">${{esc(ptTitle(e))}}</h2><div class="featured-tags">${{tags.map(x=>`<span class="tag">${{esc(x)}}</span>`).join("")}}</div><div class="featured-actions"><button class="featured-save" type="button" data-status="${{liked?"":"saved"}}" data-entry="${{esc(e.entry_id)}}" aria-label="${{t("save")}}"><span class="btn-ico">${{liked?"★":"☆"}}</span></button><button class="primary" type="button" data-detail="${{esc(e.entry_id)}}"><span class="btn-ico">⌕</span><span>${{lang==="zh"?"具体查看":"Ver detalhes"}}</span></button><button class="featured-next" type="button" data-feature-next><span class="btn-ico">→</span><span>${{lang==="zh"?"下一个脚本":"Próximo"}}</span></button></div></div></article><button class="view-all-card" type="button" data-go="all-scripts"><b>${{lang==="zh"?"查看全部推荐脚本":"Ver todos os roteiros recomendados"}}</b><span>${{lang==="zh"?"打开双列瀑布流，集中浏览全部脚本。":"Abrir a lista em duas colunas para explorar tudo."}}</span></button></section>`}}
