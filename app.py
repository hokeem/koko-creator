...
function hydrateVideo(e){{if(!e.video_url)return;setTimeout(()=>{{const box=document.querySelector(`[data-video-box="${{e.entry_id}}"]`);if(!box||box.querySelector("iframe"))return;box.innerHTML=`<iframe src="${{esc(e.video_url)}}" title="video preview" loading="lazy" allow="autoplay; encrypted-media; fullscreen; picture-in-picture" sandbox="allow-scripts allow-same-origin allow-popups allow-presentation"></iframe><div class="video-fallback">${{lang==="zh"?"如果平台禁止内嵌播放，这里可能只显示空白。":"Se a plataforma bloquear embed, o preview pode aparecer em branco."}}</div>`}},800)}}
function renderDetail(e){{const s=statusOf(e.entry_id);document.querySelector("#detail").innerHTML=`<div class="detail-top"><button class="icon" data-close>×</button></div>${{videoPreview(e)}}<h2 class="detail-title">${{esc(e.title)}}</h2><div class="tags"><span class="tag">${{esc(e.content_type)}}</span><span class="tag">1-3 min</span>${{s?`<span class="tag">${{s}}</span>`:""}}</div><div class="social-actions"><button class="social-btn" type="button" data-status="${{s==="saved"?"":"saved"}}" data-entry="${{esc(e.entry_id)}}" aria-label="${{t("save")}}">♡<span>${{s==="saved"?t("saved"):t("save")}}</span></button><button class="social-btn" type="button" data-copy-share="${{esc(e.entry_id)}}" aria-label="${{lang==="zh"?"复制分享链接":"Copiar link"}}">↗<span>${{lang==="zh"?"复制链接":"Copiar link"}}</span></button></div><div id="script-html-slot">${{e.script_html?`<article class="script-html">${{e.script_html}}</article>`:`<section class="script-loading">${{lang==="zh"?"脚本加载中..." :"Carregando roteiro..."}}</section>`}}</div><section class="submit"><b>${{t("submitTitle")}}</b><p class="lead">${{t("submitHint")}}</p><input type="url" data-submit-url="${{esc(e.entry_id)}}" placeholder="${{t("submitPlaceholder")}}"><button class="primary" data-submit="${{esc(e.entry_id)}}">${{t("submitButton")}}</button><div id="submit-status-${{esc(e.entry_id)}}"></div></section>`}}
function loadDetailHtml(e){{if(e.script_html)return;setTimeout(async()=>{{try{{const html=await fetchScriptHtml(e.entry_id);const slot=document.querySelector("#script-html-slot");if(slot)slot.innerHTML=html?`<article class="script-html">${{html}}</article>`:`<article class="script-html"><p>${{esc(e.summary)}}</p></article>`}}catch(err){{const slot=document.querySelector("#script-html-slot");if(slot)slot.innerHTML=`<article class="script-html"><p>${{esc(e.summary||err.message)}}</p></article>`}}}},5000)}}
